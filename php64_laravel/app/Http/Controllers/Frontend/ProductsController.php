...
class ProductsController extends Controller
{
    ...
    public function category($category_id){
        ...
        $order = request('order');
        $data = DB::table("products")->where("category_id","=",$category_id)->orderBy("id","desc")->paginate(4);
        $order = request('order');
        switch ($order){
            case 'nameAsc':
                $data = DB::table("products")->where("category_id","=",$category_id)->orderBy("name","asc")->paginate(4);
            break;
             case 'nameDesc':
                $data = DB::table("products")->where("category_id","=",$category_id)->orderBy("name","desc")->paginate(4);
            break;
             case 'priceAsc':
                $data = DB::table("products")->where("category_id","=",$category_id)->orderBy("price","desc")->paginate(4);
            break;
             case 'priceDesc':
                $data = DB::table("products")->where("category_id","=",$category_id)->orderBy("price","desc")->paginate(4);
            break;
    ...
    public function rating($id){
     ...
     public function searchPrice(Request $request){
        ...
        $data =DB::table("products")->where("price",">=",$promPrice)->where("price","<=",$toPrice)->paginate(40);
